  ...
  userStatus = (listNum) => {
    ...
    if (listNum > 10) {
      return (
        <div>
          <h4>Legit BudGitter</h4>
          <p>
            Add {10 - listNum} more BudGit{10 - listNum === 1 ? null : "z"} to
    ...
    return (
      <div>
        <h4>Lil BudGitter</h4>
        <p>
          Add {10 - listNum} more BudGit{10 - listNum === 1 ? null : "z"} to get
          to Legit status
        </p>
      </div>
    );
    ...
    return (
      <ul className="list">
        <li className="list-item">
          You have {lists.length} BudGit{lists.length === 1 ? null : "z"}
        </li>
        <li className="list-item">Your Status:</li>
        {this.userStatus(lists.length)}
        <li className="list-item">
          <Link onClick={this.handleLogoutClick} to="/login" className="logout">
            Logout
          </Link>{" "}
        </li>
      </ul>
